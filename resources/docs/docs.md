...
The `home-routes` handler in the `session-command.routes.home` namespace
...
The `home-page` function will in turn call the `session-command.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `session-command.handler` namespace:
...
Request middleware functions are located under the `session-command.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `session-command.dev-middleware` namespace found in
